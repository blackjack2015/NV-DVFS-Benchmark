                              ...
                              const unsigned int iterations, const int secs, const int timeRestrict)
{
    ...
    for (int iter = -1; iter < nIter; ++iter)
    {
		...
		if (iter == -1)
		{
			float msecTotal = sdkGetTimerValue(&timer_total);
    ...
    printf("Average time step 1: %f ms\n",
           sdkGetTimerValue(&timer_step1) / (float) nIter);
	printf("Total   time step 1: %f ms\n",
		   sdkGetTimerValue(&timer_step1));
    printf("Average time step 2, one intervals: %f ms\n",
		sdkGetTimerValue(&timer_step2_one) / (float)nIter);
	printf("Total   time step 2, one intervals: %f ms\n",
		sdkGetTimerValue(&timer_step2_one));
    printf("Average time step 2, mult intervals: %f ms\n",
		sdkGetTimerValue(&timer_step2_mult) / (float)nIter);
	printf("Total   time step 2, mult intervals: %f ms\n",
		sdkGetTimerValue(&timer_step2_mult));

    printf("Average time TOTAL: %f ms\n",
		sdkGetTimerValue(&timer_total) / (float)nIter);
	printf("Total   time TOTAL: %f ms\n",
		sdkGetTimerValue(&timer_total));
