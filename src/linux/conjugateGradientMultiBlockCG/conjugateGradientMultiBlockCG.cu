...
void cpuConjugateGrad(int *I, int *J, float *val, float *x, float *Ax, float *p,
  ...
  float r1 = (float)dotProduct(r, r, N);
  ...
  while (r1 > tol * tol && k <= max_iter) {
    ...
    float dot = (float)dotProduct(p, Ax, N);
    a = r1 / dot;
...
int main(int argc, char **argv) {
  ...
  *dot_result = 0.0;
  ...
  double totalTime = 0.0, gpuTime = 0.0;

  for (int i = -5; i < nIter; i++)
  {
  ...
  printf("Total   Kernel Time gpuConjugateGradient() time: %f msec\n", totalTime);
  printf("Average Kernel Time gpuConjugateGradient() time: %f msec\n", gpuTime);
