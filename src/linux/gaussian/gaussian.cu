...
int main(int argc, char *argv[])
{
  printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
   ...
   // }
    ...
    //end timing
	time_end = clock();
    unsigned int time_total = time_end * 1000 - time_start * 1000;
...
void ForwardSub()
{
	...
	Fan1 << <dimGrid, dimBlock >> >(m_cuda, a_cuda, Size, 0);
	...
	while (k < nIter)
	{
		...
		Fan2 << <dimGridXY, dimBlockXY >> >(m_cuda, a_cuda, b_cuda, Size, Size, 0);
		cudaThreadSynchronize();
	...
	// for (t=1; t<(Size-1); t++) {
	for (t=1; t<10; t++) {  // not true, just for performance test 
