...
int main(int argc, char **argv) {
  ...
  cudaMallocManaged(reinterpret_cast<void **>(&fixI), sizeof(int) * (N + 1));
  cudaMallocManaged(reinterpret_cast<void **>(&fixJ), sizeof(int) * nz);
  cudaMallocManaged(reinterpret_cast<void **>(&fixVal), sizeof(float) * nz);

  genTridiag(I, J, val, N, nz);
  cudaMemcpy(fixI, I, sizeof(int) * (N + 1), cudaMemcpyDefault);
  cudaMemcpy(fixJ, J, sizeof(int) * nz, cudaMemcpyDefault);
  cudaMemcpy(fixVal, val, sizeof(int) * nz, cudaMemcpyDefault);
  ...
  *dot_result = 0.0;
  ...
  while (k < nIter)
  {
        ...
        *dot_result = 0.0;
	printf("%.4f, %.4f.\n", val[5], fixVal[5]);
        cudaMemcpy(I, fixI, sizeof(int) * (N + 1), cudaMemcpyDefault);
        cudaMemcpy(J, fixJ, sizeof(int) * nz, cudaMemcpyDefault);
        cudaMemcpy(val, fixVal, sizeof(int) * nz, cudaMemcpyDefault);
        cudaDeviceSynchronize();
	printf("%.4f, %.4f.\n", val[5], fixVal[5]);
  ...
  checkCudaErrors(cudaDeviceSynchronize());
  getLastCudaError("conjugateGradient execution failed.\n");
  
  //sdkStopTimer(&hTimer);
  gpuTime = totalTime;
  
  printf("conjugateGradient() iterated %d, average time is %f msec.\n", nIter, gpuTime / (double)nIter);
